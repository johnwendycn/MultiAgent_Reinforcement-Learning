...
class SpatialGraphTransformer(nn.Module):
    ...
    Uses einops or native PyTorch broadcasting to compute translation-invariant
    relative coordinate embeddings between agents.

    Input shape: (batch_size, num_agents, 2)
    Output shape: (batch_size, num_agents, num_agents, embed_dim)
    """
    ...
    def compute_relative_displacements(self, positions: torch.Tensor) -> torch.Tensor:
        ...
        if einops is not None:
            ...
            # Native PyTorch broadcasting:
            # positions.unsqueeze(1): (B, 1, N, 2)
            # positions.unsqueeze(2): (B, N, 1, 2)
